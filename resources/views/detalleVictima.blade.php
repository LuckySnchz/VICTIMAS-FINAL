...
<!B7 Tipo documentación>
    ...
    @if ($tienedoc == 1) <div id="que_doc" {{ $errors->has('tipodocumento') ? 'has-error' : ''}}>
    @else
    ...
    @if ($tipodocumento == 9) <div id="cual_b2" {{ $errors->has('tipo_documento_otro') ? 'has-error' : ''}}>
    @else
...
<!B8 Nro documento>
  ...
  @if ($tienedoc == 1) <div id="que_doc8" {{ $errors->has('tipodocumento') ? 'has-error' : ''}}>
    @else
...
@if($necesidades_socioeconomicas_insatisfechas == 1)
...
@else
 ...
 @if ($victim->necesidadesIds()->contains(8))
...
<label>B 12 I. ¿Cuál?  </label><br>
 ...
 @if ($victim->programasIds()->contains(5))
...
@endif
   ...
   @if ($victim->limitacionesIds()->contains(4))
